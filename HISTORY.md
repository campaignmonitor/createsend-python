...
* Breaking: Following endpoints now return ListJoinedDate for each subscriber.***REMOVED***Also, Date has been renamed to StateChangedDate:
***REMOVED**** List.Active()
***REMOVED**** List.Bounced()
***REMOVED**** List.Unsubscribed()
***REMOVED**** List.Unconfirmed()
***REMOVED**** List.Deleted()
***REMOVED**** Segment.Subscribers()
